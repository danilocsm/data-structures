...
int dllInterlay(DLList *l1,DLList *l2)
{
	...
	if(l1!=NULL && l2!=NULL){
		...
		if(tam1>tam2 || tam1==tam2){
			curr1=l1->first; //curr1 aponta pra lista que vai ser expandida enquanto curr2 aponta pra lista que
		...
		while(curr2!=NULL){
			aux2=curr2;	 //as auxiliares guardam o nodulo que vão sofrer modificações, pois ao mover o ponteiro curr
			aux1=curr1;  //se perderia as informações necessarias para inserir o node.
			curr1=curr1->next;
			curr2=curr2->next;
			aux2->prev=aux1;
			aux2->next=aux1->next;
