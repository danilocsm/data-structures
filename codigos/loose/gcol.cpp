...
gCol *colCreate(int max)
{
    ...
    if (tam>0)
    {
        ...
        if(c != NULL)
        {
            c->vet = (void**)malloc(sizeof(void*)*tam);  //Alocando o vetor dentro da coleção;
            if (c->vet != NULL)
            {	
            	c->curr=-1;
                c->tam = tam;
...
int colInsert (gCol *c,void* key)
{
    if (c != NULL)
    {
        if (c->vet != NULL)
        {
            if (c->nelm < c->tam)
            {
            c->vet[c->nelm]= elm;    //Insere o novo elemento na posição que é a quantidade atual de elementos.
